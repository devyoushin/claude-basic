...
1. **규칙 우선**: `docs/rules/` 디렉토리의 32개 규칙을 항상 참조
2. **토큰 효율**: 문서 작성 자체도 토큰 효율적으로 진행
3. **실용적 예시**: 실제 실행 가능한 프롬프트 예시 포함
4. **TokenOps 연계**: 모든 작업 후 `ops/log.md` 업데이트 상기

## 참조 규칙

- `docs/rules/token-rules.md` — T1~T5: 토큰 절약
- `docs/rules/prompt-rules.md` — P1~P5: 프롬프트 작성
- `docs/rules/agent-rules.md` — A1~A5: Agent 활용
- `docs/rules/caching-rules.md` — C1~C5: 캐싱
- `docs/rules/workflow-rules.md` — W1~W5: 워크플로우
- `docs/rules/mistakes.md` — M1~M7: 흔한 실수
